...
public class ActivityChinese extends BaseActivity {
	...
	public void onClick(View v) {
		...
		case R.id.layout_save:
			if (!cbx_best.isChecked() && !cbx_qualied.isChecked() && !cbx_unqualied.isChecked()) {
				Util.createToast(this, "请选择普通话测试考试成绩", 3000).show();
				return;
			}
			String score = "";
			...
			try {
				AsyncMandarinScore(GlobalData.curStudent.ConfirmNo,score);
			} catch (Exception e) {
	...
	public void bindData() {
		// TODO Auto-generated method stub
		iv_menu_type.setImageResource(R.drawable.icon_chinese);
		tv_menu_type.setText("普通话测试");
	...
	AsyncHttpCallBack cb_query_student_info = new AsyncHttpCallBack() {
		...
		public void onSuccess(String response) {
			...
			try {
				...
				Util.createToast(ActivityChinese.this, obj.optString("message"), 3000).show();
	...
	private void AsyncGetStudent(String studentNo) throws Exception {
		new SwzfHttpHandler(cb_query_student_info, ActivityChinese.this).getStudent(studentNo);
	}

	private void AsyncMandarinScore(String confirmNo,String score) throws Exception {
		new SwzfHttpHandler(cb_query_student_info, ActivityChinese.this).mandarinScore(confirmNo,score);
	}
